...
def load_model(gpu_ids):
    ...
    if DataParallel_used ==1:
        ...
        checkpoint = torch.load('../models/model_29_vgg.ckpt', map_location=lambda storage, loc: storage)
...
def get_Realworld_coordinates(cnt_x, cnt_y, width):
    ...
    cnt_x_mm = map(cnt_x_mm, 0,224, 0, 600)
    cnt_y_mm = map(cnt_y_mm, 0,224, 0, 600)
    width_mm = map(width_mm, 0,224, 0, 600)
    ...
    with torch.no_grad():        
       ...
       # for index in range(1):
        ...
        if(method_access == 1):
            ...
            #img = io.imread('/home/pranay/robotic-grasping-cornell-new/dataset/grasp/Images_Test/Cropped320_rgd/pcd0102r_preprocessed_1.png')
            img = io.imread('~/catkin_ws/img_rgd_screwdriver')

            #img = img[0:700 ,0:700]   #y,y+h   x,x+w   # for our kinect setup
        ...
        print("Results in mm: ", cnt_x_mm, cnt_y_mm, width_mm  )
